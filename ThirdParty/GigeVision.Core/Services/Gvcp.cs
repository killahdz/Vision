...
namespace GigeVision.Core.Services
{
    ...
    public class Gvcp : IGvcp
    {
        ...
        private static async Task<GvcpReply> SendGvcpCommandImpl(UdpClient socketTx, GvcpCommand command)
        {
        ...
        private static async Task<GvcpReply> SendGvcpCommand(UdpClient socketTx, GvcpCommand command)
        {
            try
            {
                return await SendGvcpCommandImpl(socketTx, command);
            }
            catch (Exception ex)
            {
                //catching IO thread aborted exceptions and SocketException due to potential race condition
                //with socket handling
                Console.WriteLine($"Error in SendGvcpCommand: {ex.Message}");
                return new GvcpReply
                {
                    Status = GvcpStatus.GEV_STATUS_ERROR,
                    IsValid = false,
                    Error = ex.Message
                };
            }
